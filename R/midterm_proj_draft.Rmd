...
trend on the above four variables. The trend plot above shows large value of 
`Volume`, so we can take log to decrease its trend.
...
time series, we can leave the model to detrend time series. For the 
...
ARIMA(4, 0, 4) is the model with the best fit in the range of (1, 4) of p and q.
...
fig.cap4 = paste0("**Figure 4.** *Residual Plot of forecasts from ", 
...
distribution. We can perform a `shapiro.test()` to check the normality of the 
result residuals.

```{r norm_test, warning = FALSE, message = FALSE}
shapiro.test(resid(ts_arima)[0:5000])
```

The small p-value reject the null hypothesis that the residuals are normal. 
One way to fix that is to take log-transform of the data, but `Return` 
variable that has negative values is not applicable. Second way is to apply 
the ARIMA model that fits t-distributed errors and not assume normality. 
Third way is to use data segmentation that takes interventions into 
consideration.
